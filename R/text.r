...
#' @param fontface character value. Defaults to 'plain'. Other standard options are 'bold', 'italic', 'bold italic'. Some font families supports additional font faces.
...
#' plot(letterObject("B"))
#' }
letterObject <- function(ch, fontfamily="Helvetica", fontsize=576, fontface='plain', dim=c(480, 480)) {
  ...
  check <- try(
    grid.text(ch, 0.5,0.5, gp=gpar(fontsize=fontsize, fontfamily=fontfamily, fontface=fontface, col="white")),
    silent=TRUE
  )
  dev.off()
  ...
  if (inherits(check, "try-error")) {
    browser()
    font_problem <- grep("object 'fontfamily' not found", check)
    if (font_problem) stop(check)
    stop("an error happened in letterObject")
  }
  readJPEG(fname)
}
