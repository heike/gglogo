...
#' @param font character describing the name of a font - use e.g the command `extrafont::fonts()`
#'to check on available fonts and installing other fonts.
...
createPolygons <- function(letters, font, fontsize = 400, dim = c(720, 720), scale = FALSE) {
  ...
  # check first, that the font is implemented
  check_font <- try({
    dot <- letterObject(".", fontfamily=font, fontsize=fontsize, dim=dim)

  }, silent = TRUE)

  if (inherits(check_font, "try-error")) {
    stop(check_font)
  }
  range_dot <- range(dot)
  if (diff(range_dot) == 0) stop(sprintf("Font '%s' not found.", font))

  # we know that the font is implemented
